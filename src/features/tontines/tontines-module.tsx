...
import { OccurrenceList, OccurrenceDetail, TurnDetail } from './occurrences-module';
import { AdhesionList, AdhesionDetail } from './adhesions-module';
...
function TontineCreate({ t }: { t: T }) {
  ...
  /** Premier cycle optionnel : réutilise exactement CycleInput (aucun nouveau champ métier) — si les dates sont renseignées, un premier cycle est chaîné après la création de la Tontine ; sinon, seule la Tontine est créée. */
  const [cycleStartDate, setCycleStartDate] = useState(''); const [cycleEndDate, setCycleEndDate] = useState(''); const [cycleExpectedTotal, setCycleExpectedTotal] = useState('');
  const [error, setError] = useState<string | undefined>();
  const createCycleMutation = useMockMutation<TontineCycle | undefined, CycleInput>({ mutationFn: (input) => tontinesService.createCycle(currentTenant.id, input) });
  const mutation = useMockMutation<Tontine, TontineInput>({
    ...
    onSuccess: async (tontine) => {
      if (cycleStartDate && cycleEndDate) {
        const cycle = await createCycleMutation.mutateAsync({ tontineId: tontine.id, cycleNumber: 1, startDate: cycleStartDate, endDate: cycleEndDate, expectedTotal: Number(cycleExpectedTotal) || 0 });
        notify.success(t('tontines', 'tontineAndCycleCreated'));
        navigate(cycle ? `/tontines/${tontine.id}/cycles/${cycle.id}` : `/tontines/${tontine.id}`);
        return;
      }
  ...
  const handleSave = () => {
    ...
    if ((cycleStartDate && !cycleEndDate) || (!cycleStartDate && cycleEndDate)) { setError(t('tontines', 'fieldRequired')); return; }
    setError(undefined);
    mutation.mutate({ name, valueType, tenantId: currentTenant.id });
  };
  const isPending = mutation.isPending || createCycleMutation.isPending;
  const hasCycle = Boolean(cycleStartDate && cycleEndDate);
  return <Page title={t('tontines', 'createTontine')} description={t('tontines', 'tontinesDescription')} actions={<Back label={t('tontines', 'backToTontines')} />}><div className="grid gap-5 lg:grid-cols-2">
    <FormSection title={t('tontines', 'general')}><div className="grid gap-4 sm:grid-cols-2"><div className="space-y-2"><Label htmlFor="tontine-name">{t('tontines', 'tontineName')}</Label><Input id="tontine-name" value={name} onChange={(event) => setName(event.target.value)} aria-invalid={Boolean(error)} /><FieldError message={error} /></div><div className="space-y-2"><Label htmlFor="tontine-value-type">{t('tontines', 'valueType')}</Label><select id="tontine-value-type" value={valueType} onChange={(event) => setValueType(event.target.value as ValueType)} className="flex h-9 w-full rounded-md border border-input bg-background px-3 text-sm"><option value="MONEY">{t('tontines', 'tontineFinancial')}</option><option value="GOODS">{t('tontines', 'tontineInKind')}</option></select></div></div></FormSection>
    <PermissionGate permission="cycles.create"><FormSection title={t('tontines', 'cycleOrgSection')} description={t('tontines', 'cycleOrgDescription')}><Label className="text-xs font-medium text-foreground">{t('tontines', 'createInitialCycle')}</Label><div className="grid gap-4 sm:grid-cols-3"><div className="space-y-2"><Label htmlFor="tontine-cycle-start">{t('tontines', 'startDate')}</Label><Input id="tontine-cycle-start" type="date" value={cycleStartDate} onChange={(event) => setCycleStartDate(event.target.value)} /></div><div className="space-y-2"><Label htmlFor="tontine-cycle-end">{t('tontines', 'endDate')}</Label><Input id="tontine-cycle-end" type="date" value={cycleEndDate} onChange={(event) => setCycleEndDate(event.target.value)} /></div><div className="space-y-2"><Label htmlFor="tontine-cycle-total">{t('tontines', 'expectedTotal')}</Label><Input id="tontine-cycle-total" type="number" value={cycleExpectedTotal} onChange={(event) => setCycleExpectedTotal(event.target.value)} /></div></div><p className="text-xs text-muted-foreground">{t('tontines', 'skipInitialCycle')}</p></FormSection></PermissionGate>
    <FormSection title={t('tontines', 'adhesionsInfoSection')} description={t('tontines', 'adhesionsInfoDescription')}><></></FormSection>
    <FormSection title={t('tontines', 'summarySection')}><div className="grid gap-2 text-sm sm:grid-cols-2"><Info label={t('tontines', 'summaryName')} value={name || '—'} icon={Landmark} /><Info label={t('tontines', 'valueType')} value={valueType === 'MONEY' ? t('tontines', 'tontineFinancial') : t('tontines', 'tontineInKind')} icon={ClipboardList} /><Info label={t('tontines', 'summaryTenant')} value={`${currentTenant.name} (${currentTenant.id})`} icon={UsersRound} /><Info label={t('tontines', 'summaryFirstCycle')} value={hasCycle ? t('tontines', 'yes') : t('tontines', 'no')} icon={CalendarDays} />{hasCycle && <><Info label={t('tontines', 'startDate')} value={cycleStartDate} icon={CalendarDays} /><Info label={t('tontines', 'endDate')} value={cycleEndDate} icon={CalendarDays} /><Info label={t('tontines', 'expectedTotal')} value={formatFCFA(Number(cycleExpectedTotal) || 0)} icon={Banknote} /></>}</div></FormSection>
...
function TontineDetail({ t }: { t: T }) {
  ...
  return <Page title={tontine.name} description={`${tontine.id} · ${tontine.tenantId}`} actions={<><Back label={t('tontines', 'backToTontines')} /><Button variant="outline" onClick={() => navigate(`/tontines/${tontine.id}/adhesions`)}><UsersRound size={16} />{t('tontines', 'adhesions')}</Button><Button variant="outline" onClick={() => navigate(`/tontines/${tontine.id}/contributions`)}><ClipboardList size={16} />{t('tontines', 'contributions')}</Button><PermissionGate permission="cycles.create"><Button onClick={() => navigate(`/tontines/${tontine.id}/cycles/create`)}><Plus size={16} />{t('tontines', 'createCycle')}</Button></PermissionGate></>}><div className="grid gap-4 sm:grid-cols-5"><Metric label={t('tontines', 'valueType')} value={tontine.valueType === 'MONEY' ? t('tontines', 'tontineFinancial') : t('tontines', 'tontineInKind')} icon={ClipboardList} tone={tontine.valueType === 'MONEY' ? 'info' : 'neutral'} /><Metric label={t('tontines', 'memberCount')} value={formatNumber(tontine.memberCount)} icon={UsersRound} /><Metric label={t('tontines', 'activeCycles')} value={formatNumber(tontine.activeCycles)} icon={CalendarDays} tone="success" /><Metric label={t('tontines', 'totalContributions')} value={formatFCFA(tontine.totalContributions, 'fr', true)} icon={Banknote} tone="warning" /><Metric label={t('tontines', 'tontineStatus')} value={t('tontines', tontine.status)} icon={ShieldCheck} tone="neutral" /></div>{cycles.length === 0 && <Card className="border-primary/30 bg-primary/5"><CardContent className="flex flex-wrap items-center justify-between gap-4 p-5"><div><p className="text-sm font-semibold">{t('tontines', 'nextSteps')}</p><p className="text-xs text-muted-foreground">{t('tontines', 'nextStepsDescription')}</p></div><div className="flex flex-wrap gap-2"><PermissionGate permission="cycles.create"><Button size="sm" onClick={() => navigate(`/tontines/${tontine.id}/cycles/create`)}><Plus size={15} />{t('tontines', 'createCycle')}</Button></PermissionGate><Button size="sm" variant="outline" onClick={() => navigate(`/tontines/${tontine.id}/adhesions`)}><UsersRound size={15} />{t('tontines', 'adhesions')}</Button></div></CardContent></Card>}<Card><CardHeader className="flex-row items-center justify-between"><CardTitle className="text-sm">{t('tontines', 'cycles')}</CardTitle><Button variant="outline" size="sm" onClick={() => navigate(`/tontines/${tontine.id}/cycles`)}>{t('tontines', 'viewDetail')}<ChevronRight size={14} /></Button></CardHeader><CardContent className="p-0"><DataTable columns={columns} rows={cycles} empty={<EmptyState icon={CalendarDays} title={t('tontines', 'noCycles')} />} /></CardContent></Card></Page>;
}
...
function CycleDetail({ t }: { t: T }) {
  const { cycleId = '' } = useParams(); const navigate = useNavigate(); const { currentTenant } = useTenant(); const { data: cycle, isLoading, isError, refetch } = useCycle(cycleId);
...
export function TontinesModule() {
  ...
  return (
    <Routes>
      ...
      <Route path=":tontineId/adhesions" element={<AdhesionList t={t} />} />
      <Route path=":tontineId/adhesions/:adhesionId" element={<AdhesionDetail t={t} />} />
      ...
      <Route path=":tontineId/cycles/:cycleId/occurrences" element={<OccurrenceList t={t} />} />
      <Route path=":tontineId/cycles/:cycleId/occurrences/:occurrenceId" element={<OccurrenceDetail t={t} />} />
      <Route path=":tontineId/cycles/:cycleId/occurrences/:occurrenceId/turn" element={<TurnDetail t={t} />} />
