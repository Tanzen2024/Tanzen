...
import { currencies, DEFAULT_CURRENCY_CODE, getCurrencyLabel } from '@/constants/currencies';
...
function TontineCreate({ t }: { t: T }) {
  ...
  const handleSave = () => {
    let hasError = false;
    if (!name.trim()) { setError(t('tontines', 'fieldRequired')); hasError = true; } else setError(undefined);
    ...
    mutation.mutate({
      ...
      ...freq,
      unit: isGoods ? (unit as TontineInput['unit']) : undefined,
    });
  };
  ...
  return <Page title={tontine.name} description={`${tontine.id} · ${tontine.tenantId}`} actions={<><Back label={t('tontines', 'backToTontines')} /><Button variant="outline" onClick={() => navigate(`/tontines/${tontine.id}/contributions`)}><ClipboardList size={16} />{t('tontines', 'contributions')}</Button></>}>
    <Tabs defaultValue="overview" className="min-w-0">
      ...
      <TabsContent value="periods">
        <div className="mb-3 flex justify-end"><PermissionGate permission="cycles.create"><Button size="sm" onClick={() => navigate(`/tontines/${tontine.id}/periods/create`)}><Plus size={15} />{t('tontines', 'newPeriod')}</Button></PermissionGate></div>
