 ...
 * Tenant→Tontine→Adhésions→Périodes→Occurrences), remplace le rôle de
 * `TontineCycle` comme parent d'Occurrence. `TontineCycle` (Draw/Winner/
 * CycleMember/CycleContribution, `tontine-cycles.ts`) reste une structure
 * légataire totalement séparée et inchangée — décision explicite prise avec
 * l'utilisateur (AskUserQuestion) : Cycle et Période coexistent, aucune
 * fusion. Une Tontine est permanente et n'est jamais recréée ; ses Périodes
 * successives portent son historique temporel (D-TON refonte §2/§5).
 */
